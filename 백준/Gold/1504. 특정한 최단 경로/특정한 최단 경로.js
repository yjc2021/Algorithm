...
input.slice(1,1+e).forEach(line => {
  const [a,b,c] = line.split(' ').map(Number);
  adj[a].push([b,c]);
  adj[b].push([a,c]);
})
const [a,b] = input[1+e].split(' ').map(Number);
...
const ans1 = solution(1,a) + solution(a,b) + solution(b,n);
const ans2 = solution(1,b) + solution(b, a) + solution(a,n);
